function find_attractors_from_gcm(gcm::GeneralizedCellMap, gp::GcmProblem) :: BasinResult
    bp = gp.base
    ...
    for start_cell in 1:total_cells
        ...
        for step in 1:max_steps
            ...
            next_cell = dominant_image(gcm, current)
...
# Devolve a célula-imagem de maior probabilidade de transição. É o análogo, no
# mapa probabilístico, do ponteiro único scm.target[cell] usado no SCM.
function dominant_image(gcm::GeneralizedCellMap, cell::Int64) :: Int64
    images = images_of(gcm, cell)
    counts = counts_of(gcm, cell)
    return Int64(images[argmax(counts)])
end

#function trace_trajectory_in_map(u, region::mapGeneralizedCell) :: Int64 -> Embutida em find_attractors_from_gcm por enquanto

#function detect_cycle_in_trajectory(trajectory::Vector{Int64}) :: Union{Int64, Nothing} -> Embutida em find_attractors_from_gcm por enquanto

#function classify_attractor_type(u, region::mapGeneralizedCell) :: Int64 -> Embutida em find_attractors_from_gcm por enquanto

#function mark_basin_cells(trajectory, attractor_id, grid) -> Embutida em find_attractors_from_gcm por enquanto